class Solution {
    ...
    private void dfs(int node, int parent, ArrayList<ArrayList<Integer>> adjList, int[] dis, int[] minDis, int[] timer, List<List<Integer>> ans) {
        minDis[node] = dis[node] = timer[0]++;
        for(int adj : adjList.get(node)) {
            if( adj == parent ) continue;
            if( dis[adj] == 0 ) dfs(adj, node, adjList, dis, minDis, timer, ans);
